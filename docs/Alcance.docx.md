...
0. #### **Gestión del Proyecto**
   ...
   4. Documento de Alcance y Gestión (este documento)

#### **2.0 Diseño y Arquitectura**

1. Definición de stack tecnológico

   2. Modelo de datos genérico multirubro (RNF-01 a RNF-03)

   3. Spike técnico de homologación con Web Services de ARCA (mitigación de riesgo, Acta

y Riesgos)

3. #### **0 Módulo Productos y Stock**

   1. Alta/baja/modificación de productos (RF-01)

   2. Categorías y variantes configurables (RF-02, RF-03)

   3. Control de inventario y ajustes de stock (RF-04)

   4. Alertas de stock mínimo (RF-05)

4. #### **0 Módulo Facturación (ARCA)**

   1. Integración WSAA/WSFEv1 — ambiente de homologación (RF-06, RF-08)

   2. Emisión de Factura A/B/C (RF-06)

   3. Notas de Crédito/Débito (RF-07)

   4. Manejo de errores de WS y logging seguro (RF-09, RF-10, RNF-04, RNF-05)

   5. Exportación de comprobantes en PDF (RF-11)

   6. Habilitación de credenciales de producción configurables (RF-08)

5. #### **0 Módulo Clientes/Proveedores y Cuentas Corrientes**

   1. ABM de clientes y proveedores con datos fiscales (RF-12)

   2. Cuentas corrientes: saldo, movimientos, límite de crédito (RF-13)

   3. Consulta de estado de cuenta (RF-14)

6. #### **0 Módulo Pagos y Giftcards**

   1. Medios de pago múltiples: efectivo, débito/crédito, transferencia, QR, cuenta corriente (RF-15)

   2. Pago combinado / mixto (RF-16)

   3. Giftcards: emisión, recarga, canje, consulta de saldo (RF-17 a RF-20)

   4. Integración con pasarela simulada/sandbox (RF-21)

7. #### **0 Módulo Reportes y Estadísticas**

   1. Reporte de ventas por período (RF-22)

   2. Reporte de productos más vendidos (RF-23)

   3. Reporte de stock crítico (RF-24)

   4. Reporte de cuentas corrientes (RF-25)

   5. Reporte de rentabilidad (RF-26)

   6. Exportación de reportes (RF-27)

8. #### **0 Módulo Usuarios y Permisos**

   1. Rol administrador (RF-29)

   2. Rol cajero (RF-30)

9. #### **0 Pruebas, Integración y Cierre**

   1. Prueba de ciclo integral: alta de producto → venta → facturación → pago → reportes (RF-31)

   2. Validación de genericidad en ≥2 rubros (RF-32)

   3. Validación final contra ambiente de homologación de ARCA (Acta y Criterios de éxito)

   4. Entrega final y documentación de cierre
...
6. ### **Criterios de éxito y restricciones**
   ...
   1. #### **Criterios de éxito**
      ...
      * El sistema permite operar el ciclo completo: alta de producto → venta con control de stock →

facturación → registro de pago → impactó en reportes. *(RF-31)*
